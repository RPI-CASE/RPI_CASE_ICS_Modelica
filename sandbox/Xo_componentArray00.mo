model Xo_componentArray00 "Modeling discretized rod"
  ...
  parameter Modelica.SIunits.HeatCapacity C = 100 "Total heat capacity of element (= cp*m)";
  parameter Modelica.SIunits.ThermalConductance G_wall = 1 "Thermal conductivity of wall";
  parameter Modelica.SIunits.ThermalConductance G_rod = 200 "Thermal conductivie of rod";
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor heatcapacitor_amb(C = 1e66, T.start = 250, T.fixed = false) annotation(Placement(visible = true, transformation(origin = {-40, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  ...
  Modelica.Blocks.Sources.IntegerConstant integerConstant(k = 8) annotation(Placement(transformation(extent = {{-70, 66}, {-58, 78}})));
  Modelica.Blocks.Sources.Constant DNI(k = 1000) annotation(Placement(transformation(extent = {{-86, 62}, {-76, 72}})));
  Modelica.Thermal.FluidHeatFlow.Sources.VolumeFlow volumeFlow(m = 0.01, constantVolumeFlow(displayUnit = "l/min") = 1.6666666666667e-06) annotation(Placement(transformation(extent = {{-54, 36}, {-44, 46}})));
  Modelica.Blocks.Sources.Constant const(k = 8) annotation(Placement(visible = true, transformation(origin = {-20, 80}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  ...
  Modelica.Thermal.HeatTransfer.Components.HeatCapacitor heatcapacitor_sink(C = 1e66, T.start = 275, T.fixed = false) annotation(Placement(visible = true, transformation(origin = {60, -40}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  Modelica.Blocks.Sources.Constant constant1(k = 0.5) annotation(Placement(visible = true, transformation(origin = {-65, 15}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant constant2(k = 0.9) annotation(Placement(visible = true, transformation(origin = {-60, 0}, extent = {{-5, -5}, {5, 5}}, rotation = 0)));
protected
...
equation
  connect(constant2.y, valve2.y) annotation(Line(points = {{-54.5, 0}, {-22.2222, 0}, {-22.2222, -16.4875}, {0.238949, -16.4875}, {0.238949, -9.79689}, {0.238949, -9.79689}}, color = {0, 0, 127}));
  connect(constant1.y, valve1.y) annotation(Line(points = {{-59.5, 15}, {-0.238949, 15}, {-0.238949, 10.9916}, {-0.238949, 10.9916}}, color = {0, 0, 127}));
  ...
  connect(valve1.flowPort_a, iCS_Stack.flowport_a1) annotation(Line(points = {{10, 20}, {16.2485, 20}, {16.2485, 39.9044}, {-28.4349, 39.9044}, {-28.4349, 53.2855}, {-14.3369, 53.2855}, {-14.3369, 53.2855}}, color = {255, 0, 0}));
  connect(volumeFlow.flowPort_b, valve2.flowPort_b) annotation(Line(points = {{-44, 41}, {-33.9307, 27.8578}, {-25.5675, 15.7706}, {-10, 0}, {-10, 0}}, color = {255, 0, 0}));
  connect(volumeFlow.flowPort_b, valve1.flowPort_b) annotation(Line(points = {{-44, 41}, {-26.2843, 31.2031}, {-10.2748, 20}, {-10, 20}}, color = {255, 0, 0}));
  connect(ambient1.flowPort, volumeFlow.flowPort_a) annotation(Line(points = {{-73, 40}, {-64, 40}, {-64, 41}, {-54, 41}}, color = {255, 0, 0}));
  connect(const.y, iCS_Stack.StackHeight) annotation(Line(points = {{-14.5, 80}, {-19.8327, 80}, {-19.8327, 61.8877}, {-14.3369, 61.8877}, {-14.3369, 61.8877}}, color = {0, 0, 127}));
  for i in 1:n loop
  ...
  for i in 1:n - 1 loop
    connect(capacitance[i].port, rod_conduction[i].port_a) "Capacitance to next conduction";
    connect(capacitance[i + 1].port, rod_conduction[i].port_b) "Capacitance to prev conduction";
  end for;
  connect(capacitance[1].port, fixedTemperature.port) "First capacitance to rod end";
  connect(capacitance[n].port, heatcapacitor_sink.port) "Last capacitance to (other) rod end";
  ...
  connect(integerConstant.y, iCS_Stack.StackHeightInt) annotation(Line(points = {{-57.4, 72}, {-44, 72}, {-44, 64.4}, {-14.4, 64.4}}, color = {255, 127, 0}, smooth = Smooth.None));
  connect(DNI.y, iCS_Stack.DNI) annotation(Line(points = {{-75.5, 67}, {-44.75, 67}, {-44.75, 58}, {-14, 58}}, color = {0, 0, 127}, smooth = Smooth.None));
  annotation(uses(Modelica(version = "3.2.1"), Buildings(version = "1.6")), Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-100, -100}, {100, 100}}), graphics));
