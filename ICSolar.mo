package ICSolar "Integrated Concentrating Solar simulation, packaged for hierarchy construction"
  ...
  package Envelope "Package of all the Envelope components to create an Integrated Concentrating Solar simulation"
    ...
    model GlazingTransmittance_ETFE
      ...
      Real c_disp = 0.06 "Coefficient of dispersion/scatter adjusted to reach 94% optical transmittance at AOI = 0";
      constant Real n_air = 1.0 "optical index, air";
  ...
  package Module "Package of all the necessary components to create an Integrated Concentrating Solar simulation"
    ...
    model ICS_PVPerformance "This model uses the EIPC (based on cell area) and PVEfficiency (based on ConcentrationFactor) to calculate the ElectricalGen and ThermalGen"
      ...
      Real CellEfficiency = 0.447 * (1 + k_Isc * (ThermalGen.T-(273.15+22.5))) * (1 + k_Voc * (ThermalGen.T-(273.15+22.5))) * (1 + k_FF * (ThermalGen.T-(273.15+22.5)));
  ...
  model Parameters "Inputs for the ICSF model"
    ...
    parameter Real OneBranchFlow = 5e-006;
  ...
  model testModule
    ...
    Modelica.Thermal.FluidHeatFlow.Sources.VolumeFlow volumeflow1(constantVolumeFlow = 0.0005999999999999999, m = 1, T0 = 300) annotation(Placement(visible = true, transformation(origin = {-20,40}, extent = {{-10,-10},{10,10}}, rotation = 0)));
  ...
  equation
    connect(fixedtemperature1.port,ics_module_twelve1.TAmb_in) annotation(Line(points = {{-50,80},{-33.0176,80},{-33.0176,7.03654},{-9.742900000000001,7.03654},{-9.742900000000001,7.03654}}, color = {191,0,0}));
