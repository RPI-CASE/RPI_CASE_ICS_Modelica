package ICSolar "Package of all the necessary components to create an Integrated Concentrating Solar simulation"
  ...
  equation
    connect(ics_envelopecassette1.flowport_b,Sink.flowPort) annotation(Line(points = {{25,25},{41.2098,25},{41.2098,0.378072},{49.1493,0.378072},{49.1493,0.378072}}));
    connect(Pump.flowPort_b,ics_envelopecassette1.flowport_a) annotation(Line(points = {{-30,-20},{-28.3554,-20},{-28.3554,19.6597},{-22.6843,19.6597},{-22.6843,19.6597}}));
    connect(ics_context1.DNI,ics_envelopecassette1.DNI) annotation(Line(points = {{-55,25},{-24.1966,25},{-24.1966,23.8185},{-24.1966,23.8185}}));
    connect(ics_context1.AOI,ics_envelopecassette1.AOI) annotation(Line(points = {{-55,30},{-24.9527,30},{-24.9527,31.0019},{-24.9527,31.0019}}));
    connect(ics_context1.SunAzi,ics_envelopecassette1.SunAzi) annotation(Line(points = {{-55,35},{-24.9527,35},{-24.9527,35.5388},{-24.9527,35.5388}}));
    connect(ics_context1.SunAlt,ics_envelopecassette1.SunAlt) annotation(Line(points = {{-55,40},{-24.5747,40},{-24.5747,39.6975},{-24.5747,39.6975}}));
    connect(ics_context1.SurfOrientation_out,ics_envelopecassette1.SurfaceOrientation) annotation(Line(points = {{-55,45},{-25.7089,45},{-25.7089,44.9905},{-25.7089,44.9905}}));
    connect(ics_context1.SurfTilt_out,ics_envelopecassette1.SurfaceTilt) annotation(Line(points = {{-55,50},{-25.7089,50},{-25.7089,49.5274},{-25.7089,49.5274}}));
    connect(ics_context1.TDryBul,ics_envelopecassette1.TAmb_in) annotation(Line(points = {{-55,55},{-25.3308,55},{-25.3308,55.9546},{-25.3308,55.9546}}));
  ...
  package Envelope "Package of all the Envelope components to create an Integrated Concentrating Solar simulation"
    ...
    equation
      connect(flowport_a,ics_stack1.flowport_a1) annotation(Line(points = {{-100,-80},{3.78072,-80},{3.78072,-15.5009},{15.879,-15.5009},{15.879,-15.5009}}));
      connect(ics_selfshading1.DNI_out,ics_stack1.DNI) annotation(Line(points = {{-2.5,3.5},{6.04915,3.5},{6.04915,-4.53686},{15.1229,-4.53686},{15.1229,-4.53686}}));
      connect(TAmb_in,ics_stack1.TAmb_in) annotation(Line(points = {{-100,80},{-1.13422,80},{-1.13422,17.0132},{15.5009,17.0132},{15.5009,17.0132}}));
      connect(ics_stack1.flowport_b1,flowport_b) annotation(Line(points = {{65,0},{80.1512,0},{80.1512,-22.3062},{102.457,-22.3062},{102.457,-22.3062}}));
      connect(ics_stack1.Power_out,Power_out) annotation(Line(points = {{65,10},{80.9074,10},{80.9074,18.9036},{94.1399,18.9036},{94.1399,18.9036}}));
      connect(SurfaceTilt,ics_selfshading1.SurfTilt) annotation(Line(points = {{-100,-40},{-45.5285,-40},{-45.5285,-14.0921},{-38.2114,-14.0921},{-38.2114,-14.0921}}));
      connect(SurfaceOrientation,ics_selfshading1.SurfOrientation) annotation(Line(points = {{-100,-20},{-57.7236,-20},{-57.7236,-6.50407},{-37.1274,-6.50407},{-37.1274,-6.50407}}));
      ...
      connect(ics_glazinglosses1.SurfDirNor,ics_selfshading1.DNI_in) annotation(Line(points = {{-40,58},{-32.7913,58},{-32.7913,33.6043},{-50.1355,33.6043},{-50.1355,14.0921},{-37.9404,14.0921},{-37.9404,14.0921}}));
      connect(AOI,ics_glazinglosses1.AOI) annotation(Line(points = {{-100,40},{-83.1978,40},{-83.1978,45.7995},{-69.6477,45.7995},{-69.6477,45.7995}}));
      connect(DNI,ics_glazinglosses1.DNI) annotation(Line(points = {{-100,60},{-75.88079999999999,60},{-75.88079999999999,63.9566},{-70.1897,63.9566},{-70.1897,63.9566}}));
    ...
    equation
      connect(rotationmatrixforsphericalcood1.SurfPitch,Shading.u2) annotation(Line(points = {{-50,-44},{-40.6504,-44},{-40.6504,-46.3415},{-31.7073,-46.3415},{-31.7073,-46.3415}}));
      connect(rotationmatrixforsphericalcood1.SurfYaw,Shading.u1) annotation(Line(points = {{-50,-34},{-32.7913,-34},{-32.7913,-34.1463},{-32.7913,-34.1463}}));
      connect(SurfTilt,rotationmatrixforsphericalcood1.SurfaceTilt) annotation(Line(points = {{-100,-80},{-75.60980000000001,-80},{-75.60980000000001,-46.3415},{-69.1057,-46.3415},{-69.1057,-46.3415}}));
      connect(SurfOrientation,rotationmatrixforsphericalcood1.SurfaceOrientation) annotation(Line(points = {{-100,-60},{-78.8618,-60},{-78.8618,-42.0054},{-70.1897,-42.0054},{-70.1897,-42.0054}}));
      connect(SunAlt,rotationmatrixforsphericalcood1.SunAlt) annotation(Line(points = {{-100,-40},{-78.8618,-40},{-78.8618,-37.9404},{-70.1897,-37.9404},{-70.1897,-37.9404}}));
      connect(SunAzi,rotationmatrixforsphericalcood1.SunAzi) annotation(Line(points = {{-100,-20},{-76.6938,-20},{-76.6938,-33.8753},{-69.9187,-33.8753},{-69.9187,-33.8753}}));
  ...
  package Module "Package of all the necessary components to create an Integrated Concentrating Solar simulation"
    ...
    model ICS_PVPerformance "This model uses the EIPC (based on cell area) and PVEfficiency (based on ConcentrationFactor) to calculate the ElectricalGen and ThermalGen"
      ...
      Real CellEfficiency = 0.36436 + (52.5 - ThermalGen.T) * 0.0005004 + (ConcentrationFactor - 627.5) * 1.9965e-006 "Equation to determine the PVEfficiency from the ConcentrationFactor";
